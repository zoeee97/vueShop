...
export default {
  ...
  mounted() {
    // 组件挂载完毕执行一次【仅仅执行一次】
    /* 而需求是用户的请求参数改变时就应该再发一次请求，因此不能直接在mounted里派发actions，
    而应该封装成一个函数，谁需要用时就调用*/
    // 在发请求之前咱们需要将searchParams里面参数进行修改带给服务器
    this.getData();
  },
  computed: {
    // mapGetters里的写法，传递的是数组，因为getters计算是不划分模块的（除非开了命名空间
    ...mapGetters(["goodsList"]),
    // 开了空间命名的可以试一试这个...mapGetters('search',['goodsList'])
    isOne() {
  ...
  methods: {
    ...
    removeKeyword() {
      ...
      if (this.$route.query) {//其实这条对象会一直为真，只有对象为空指针时才会为假
